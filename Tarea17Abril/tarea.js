...
const generoSinRepetir = (listaPeliclas) => listaPeliclas.reduce((accumulator, currentPeli) => {
    if (!accumulator.includes(currentPeli.genre)) {
        accumulator.push(currentPeli.genre);
    }
    return accumulator;
}, []); 

console.log(generoSinRepetir(movies))

// 2. Obtener una lista de los nombres de los directores sin repetir

const directorsSinRepetir = (listaPeliclas) => listaPeliclas.reduce((accumulator, currentPeli) => {
    if (!accumulator.includes(currentPeli.director)) {
        accumulator.push(currentPeli.director);
    }
    return accumulator;
}, []); 

console.log(directorsSinRepetir(movies))

// 3. Obtener una lista con únicamente los nombres de los actores protagonistas de todas las películas, sin repetir

const protagonistasSinRepetir = (listaPeliclas) => listaPeliclas.reduce((accumulator, currentPeli) => {
    let protagonistas = currentPeli.protagonists.map(protagonista => protagonista.name)
    // return protagonistas
    if (!accumulator.includes(protagonistas)) {
        accumulator.push(protagonistas);
    }
    return accumulator;
}, []); 

console.log(protagonistasSinRepetir(movies))

// 4. Obtener uan lista de clasificaciones (rating) sin repetir

const ratingSinRepetir = (listaPeliclas) => listaPeliclas.reduce((accumulator, currentPeli) => {
    if (!accumulator.includes(currentPeli.rating)) {
        accumulator.push(currentPeli.rating);
    }
    return accumulator;
}, []); 

console.log(ratingSinRepetir(movies))
...
const duracionPorRango = (listaPeliclas, incio, fin) => {
    let listaPelis = []
    listaPeliclas.forEach(pelicula => {
        if (pelicula.duration.slice(0,3) >= incio && pelicula.duration.slice(0,3) <= fin){
...
console.log(duracionPorRango(movies, 100, 120))

// 6. Obtener una lista de películas con base en su clasificación

const peliculaPorRating = (listaPeliclas) => {
    let result = listaPeliclas.reduce((accum, current) => {
        return accum[current.rating]
      ? {
          ...accum,
          [current.rating]: [...accum[current.rating], current],
        }
...
console.log(peliculaPorRating(movies))
...
const estrenoPorRango = (listaPeliclas, incio, fin) => {
    let listaPelis = []
    listaPeliclas.forEach(pelicula => {
        if (pelicula.releaseYear >= incio && pelicula.releaseYear <= fin){
            listaPelis.push(pelicula)
        }
    })
    return listaPelis
...
console.log(estrenoPorRango(movies, 2010, 2015))

// 8. Obtener una lista de películas con base en el país al que pertenecen

const peliculaPorPais = (listaPeliclas) => {
    let result = listaPeliclas.reduce((accum, current) => {
        return accum[current.country]
      ? {
          ...accum,
          [current.country]: [...accum[current.country], current],
        }
...
console.log(peliculaPorPais(movies))

// 9. Obtener una lista de las películas que no obtuvieron premios Óscar

// const sinOscar = (listaPeliclas) = listaPeliclas.filter((pelicula) => pelicula.oscarAwards == 0)
 
console.log(sinOscar(movies))
...
const cantidadPorClaisficacion = (listaPeliclas) => {
    let result = listaPeliclas.reduce((acumulador, pelicula) => {
        if (!acumulador[pelicula.rating]) {
          acumulador[pelicula.rating] = 1;
        } else {
          acumulador[pelicula.rating]++;
        }
        return acumulador;
      }, {});
      return result
...
console.log(cantidadPorClaisficacion(movies))
...
const cantidadPorPais = (listaPeliclas) => {
    let result = listaPeliclas.reduce((acumulador, pelicula) => {
        if (!acumulador[pelicula.country]) {
          acumulador[pelicula.country] = 1;
        } else {
          acumulador[pelicula.country]++;
        }
        return acumulador;
      }, {});
      return result
...
console.log(cantidadPorPais(movies))

// 12. Obtener la edad promedio de los actores protagonistas

const edadPromedioProtagonistas = (listaPeliclas) =>{ 
    let sumaEdades = 0;
    let totalProtagonistas = 0;
    
    listaPeliclas.forEach(movie => {
    movie.protagonists.forEach(protagonista => {
      sumaEdades += protagonista.age;
      totalProtagonistas++;
    });
  });

  const edadPromedio = sumaEdades / totalProtagonistas;
  return edadPromedio;
}

console.log(edadPromedioProtagonistas(movies))

// 13. Dado el nombre de un actor, obtener la cantidad de películas de la lista en las que aparece

const cantidadPeliculasPorActor = (listaPeliclas, nombreActor) =>{ 
    let sumaPeliculas = 0;
    
    listaPeliclas.forEach(movie => {
    movie.protagonists.forEach(protagonista => {
        if (nombreActor == protagonista.name){
            sumaPeliculas ++
        }
    });
  });
...
console.log(cantidadPeliculasPorActor(movies, "Leonardo DiCaprio"))
...
const detailsMovies = (listaPeliclas) => listaPeliclas.reduce((accum, current) => {
        return [
            ...accum,
            {
                "title" : `${current.title} - ${current.director} - ${current.duration}`,
                "trimmedSynopsis" : current.synopsis
            },
        ]
    },[])
